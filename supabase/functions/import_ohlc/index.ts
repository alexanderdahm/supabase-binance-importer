...
const SUPABASE_URL = env.PUBLIC_SUPABASE_URL;            // renamed
const SUPABASE_SERVICE_ROLE_KEY = env.SERVICE_ROLE_KEY;   // renamed
const START_DATE = env.START_DATE;                         // e.g., "2021-01-01"
...
serve(async () => {
  ...
  const BATCH_DAYS = 1000; // Binance allows max 1000 candles per request
  let totalFetched = 0;
  let totalInserted = 0;
  let totalSkipped = 0;
  let batches = 0;
  ...
  while (current <= end) {
    ...
    if (!Array.isArray(data)) return new Response("Error fetching data from Binance", { status: 500 });
    ...
    totalSkipped += candles.length - (inserted?.length || 0);
    batches++;
